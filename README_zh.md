...
- 📊 **高性能数据表格 (Data Grid)**：
  ...
  - 支持新增行、复制行、删除行等便捷操作。
- ✨ **可视化模拟测试数据生成器 (Mock Data Generator)**：
  - 基于字段名与数据类型智能启发式推导生成策略（姓名、邮箱、手机号、UUID、数值范围、日期时间、枚举选项等）。
  - 多步向导式界面，实时生成样本数据并在表格中预览校验。
  - 多方言原子事务分批插入，实时进度条指示并在完成后自动刷新数据网格。
- 🚀 **全功能数据导出与表转储向导 (Export & Dump)**：
  - 支持多方言 SQL 转储脚本（建表 DDL + 多行批量 INSERT）、CSV、TSV、JSON、NDJSON 与 GitHub Markdown。
  - 支持字段作用域过滤、WHERE 条件截取、行数限制、本地文件直存及系统剪贴板快速复制。
- 💻 **现代化 SQL 查询控制台**：
  - 多标签页独立查询会话。
  - 集成一键 SQL 格式化与美化。
